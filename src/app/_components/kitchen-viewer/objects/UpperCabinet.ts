...
export class UpperCabinet extends KitchenObject {
  ...
  protected build() {
    ...
    // Laterales
    this.addBox(0.018, H, D, bd, -W/2 + 0.009, H/2, -D/2, "board");
    this.addBox(0.018, H, D, bd,  W/2 - 0.009, H/2, -D/2, "board");
    // Fondo
    this.addBox(W - 0.036, H, 0.018, bd, 0, H/2, -D + 0.009, "board");
    // Techo
    this.addBox(W - 0.036, 0.018, D, bd, 0, H - 0.009, -D/2, "board");
    // Piso
    this.addBox(W - 0.036, 0.018, D, bd, 0, 0.009, -D/2, "board");

    // Entrepaños
    for (let i = 1; i <= numShelves; i++) {
      const y = H * (i / (numShelves + 1));
      this.addBox(W - 0.04, 0.016, D - 0.02, bd, 0, y, 0.01-D/2, "board");
    }
    ...
    if (hasGlass) {
      ...
      if (W > 0.55) {
        ...
        this.addBox(W/2 - 0.004, doorH, 0.016, this.defaultFinishMat(),
          -W/4, doorH/2 + 0.006, 0 - 0.008, "finish");
        this.addBox(W/2 - 0.004, doorH, 0.016, this.defaultFinishMat(),
           W/4, doorH/2 + 0.006, 0 - 0.008, "finish");
        // Vidrio
        const glassA = new THREE.Mesh(new THREE.BoxGeometry(W/2 - 0.02, doorH - 0.02, 0.005), glassMat);
        glassA.position.set(-W/4, doorH/2 + 0.006, 0 - 0.001);
        this.add(glassA);
        const glassB = glassA.clone();
        glassB.position.set(W/4, doorH/2 + 0.006, 0 - 0.001);
        this.add(glassB);
      } else {
        this.addBox(W - 0.006, doorH, 0.016, this.defaultFinishMat(),
          0, doorH/2 + 0.006, 0 - 0.008, "finish");
        const glass = new THREE.Mesh(
          new THREE.BoxGeometry(W - 0.02, doorH - 0.02, 0.005), glassMat
        );
        glass.position.set(0, doorH/2 + 0.006, 0 - 0.001);
        this.add(glass);
      }
    } else {
      if (W > 0.55) {
        this.addBox(W/2 - 0.004, doorH, 0.018, this.defaultFinishMat(),
          -W/4, doorH/2 + 0.006, 0 - 0.009, "finish");
        this.addBox(W/2 - 0.004, doorH, 0.018, this.defaultFinishMat(),
           W/4, doorH/2 + 0.006, 0 - 0.009, "finish");
        this.addCylinder(0.006, 0.08, hd, -W/4, doorH * 0.3, 0+ 0.012, Math.PI/2);
        this.addCylinder(0.006, 0.08, hd,  W/4, doorH * 0.3, 0 + 0.012, Math.PI/2);
      } else {
        this.addBox(W - 0.006, doorH, 0.018, this.defaultFinishMat(),
          0, doorH/2 + 0.006, 0 - 0.009, "finish");
        this.addCylinder(0.006, 0.08, hd, 0, doorH * 0.3, 0 + 0.012, Math.PI/2);
      }
    }
  }
}