// src/components/quote-builder/LayoutGroupPanel.tsx
"use client";
...
// ✅ No import from @prisma/client — define inline
const CONNECTION_LABELS: Record<string, string> = {
  INLINE: "→", CORNER_90R: "↱ 90° der", CORNER_90L: "↰ 90° izq",
  CORNER_45: "↱ 45°", GAP: "← gap →", END: "",
};
...
const id = useId()
  ...
  // Sync items when group prop changes (after invalidate)
  if (items.length !== group.items.length ||
      items.some((it, i) => it.id !== group.items[i]?.id)) {
    setItems(group.items);
  }

  return (
    <div
      className={`rounded-lg border bg-white transition-colors dark:bg-gray-900 ${
        isSelected
          ? "border-blue-400 ring-1 ring-blue-300 dark:border-blue-500"
          : "border-gray-200 dark:border-gray-800"
      }`}
    >
      {/* Header */}
      <div
        className="flex cursor-pointer items-center gap-2 border-b border-gray-100 px-3 py-2.5 dark:border-gray-800"
      ...
      >
        ...
        <button
          onClick={e => {
            e.stopPropagation();
            if (confirm(`¿Eliminar grupo "${group.name}"?`)) deleteGroup.mutate({ id: group.id });
          }}
        ...
        <div className="space-y-1.5 p-2">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={items.map(i => i.id)} strategy={verticalListSortingStrategy} id={id}>
              {items.map((item, idx) => (
                <SortableQuoteItemWrapper key={item.id} item={item} isLast={idx === items.length - 1}  />
              ))}
            </SortableContext>
          </DndContext>
          ...
          {items.length === 0 && (
            <p className="py-4 text-center text-xs text-gray-400">
              {isSelected
                ? "Haz clic en un elemento del catálogo para agregarlo aquí"
                : "Haz clic en el grupo para seleccionarlo, luego agrega elementos"}
            </p>
          )}
        </div>
      )}
...
function SortableQuoteItemWrapper({ item, isLast }: { item: Group["items"][number]; isLast: boolean }) {
      ...
      {!isLast && (
        <div className="my-0.5 ml-5 flex items-center gap-2">
          ...
          <span className="rounded bg-gray-50 px-2 py-0.5 text-xs text-gray-400 dark:bg-gray-800/50 dark:text-gray-500">
            {CONNECTION_LABELS[item.connectionToNext] ?? item.connectionToNext}
          </span>
          <div className="h-px flex-1 bg-gray-100 dark:bg-gray-800" />
        </div>
      )}
