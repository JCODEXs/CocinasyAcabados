/* eslint-disable @typescript-eslint/no-unsafe-assignment */

// src/components/quote-builder/CatalogSidebar.tsx
"use client";

import { useState, useMemo } from "react";
import { type RouterOutputs } from "@/trpc/react";
import { api } from "@/trpc/react";
import { useQuoteBuilder } from "./context";
import { MagnifyingGlassIcon, PlusIcon } from "@heroicons/react/24/outline";
import { type ElementType } from "@prisma/client";

type Catalog = RouterOutputs["catalog"]["getFullCatalog"];

const CATEGORY_LABELS: Record<string, string> = {
  MUEBLE_BAJO:      "Muebles bajos",
  MUEBLE_ALTO:      "Muebles altos",
  MESON:            "Mesones",
  ELECTRODOMESTICO: "Electrodomésticos",
  PANEL_YESO:       "Panel yeso",
  SUPERBOARD:       "Superboard",
  PUERTA:           "Puertas",
  ESTANTE:          "Estantes",
  OTRO:             "Otros",
};

const CATEGORY_ORDER = Object.keys(CATEGORY_LABELS);

const UNIT_SUFFIX: Record<string, string> = {
  POR_ML: "/ml", POR_M2: "/m²", POR_UNIDAD: "/und",
};

export function CatalogSidebar() {
  const { catalog, projectId, invalidateProject, selectedGroupId } = useQuoteBuilder();
  const [search, setSearch] = useState("");

const filtered = useMemo((): ElementType[] => {
  if (!catalog?.elementTypes) return [];
  if (!search.trim()) return catalog.elementTypes;
  
  const q = search.toLowerCase();
  return catalog.elementTypes.filter(e => e.name.toLowerCase().includes(q));
}, [search, catalog?.elementTypes]);

const byCategory = useMemo((): Map<string, ElementType[]> => {
  const map = new Map<string, ElementType[]>();
  
  for (const et of filtered) {
    const existing = map.get(et.category);
    if (existing) {
      existing.push(et);
    } else {
      map.set(et.category, [et]);
    }
  }
  
  return map;
}, [filtered]);

  return (
    <div className="flex h-full flex-col">
      {/* Búsqueda */}
      <div className="border-b border-gray-100 p-3 dark:border-gray-800">
        <div className="relative">
          <MagnifyingGlassIcon className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-400" />
          <input
            type="text"
            placeholder="Buscar elemento..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full rounded-md border border-gray-200 bg-gray-50 py-1.5 pl-8 pr-3 text-sm placeholder-gray-400 focus:border-gray-400 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
          />
        </div>
      </div>

      {/* Indicador de grupo activo */}
      <div className={`shrink-0 border-b px-3 py-2 text-xs transition-colors ${
        selectedGroupId
          ? "border-blue-100 bg-blue-50 text-blue-700 dark:border-blue-900 dark:bg-blue-950/50 dark:text-blue-400"
          : "border-gray-100 bg-gray-50 text-gray-400 dark:border-gray-800 dark:bg-gray-800/40"
      }`}>
        {selectedGroupId
          ? "Haz clic en + para agregar al grupo activo"
          : "Selecciona un grupo para agregar elementos"
        }
      </div>

      {/* Lista de elementos */}
      <div className="flex-1 space-y-4 overflow-y-auto p-2">
        {CATEGORY_ORDER.map(cat => {
          const items = byCategory.get(cat);
          if (!items?.length) return null;
          return (
            <div key={cat}>
              <p className="mb-1.5 px-1 text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">
                {CATEGORY_LABELS[cat]}
              </p>
              <div className="space-y-1">
                {items.map(et => (
                  <ElementTypeCard
                    key={et.id}
                    elementType={et}
                    projectId={projectId}
                    selectedGroupId={selectedGroupId}
                    onAdded={invalidateProject}
                  />
                ))}
              </div>
            </div>
          );
        })}

        {filtered?.length === 0 && (
          <p className="py-8 text-center text-sm text-gray-400">Sin resultados</p>
        )}

        {catalog?.elementTypes.length === 0 && (
          <div className="py-8 text-center">
            <p className="text-sm text-gray-400">Catálogo vacío</p>
            <a href="/dashboard/catalog" className="mt-2 block text-xs text-blue-500 hover:underline">
              Ir al catálogo para agregar elementos →
            </a>
          </div>
        )}
      </div>
    </div>
  );
}

function ElementTypeCard({ elementType, projectId, selectedGroupId, onAdded }: {
  elementType: ElementType;
  projectId: string;
  selectedGroupId: string | null;
  onAdded: () => void;
}) {
  const [adding, setAdding] = useState(false);

  const addItem = api.quotes.addQuoteItem.useMutation({
    onSuccess: () => { setAdding(false); onAdded(); },
    onError:   () => { setAdding(false); },
  });


  const handleAdd = () => {
    if (!selectedGroupId) return;
    
    setAdding(true);
    addItem.mutate({
      projectId,
      elementTypeId: elementType.id,
      // componentTemplateId:elementType.componentTemplateId,
      label: elementType.name,
      width:  elementType.defaultWidth  ?? 60,
      height: elementType.defaultHeight ?? 72,
      depth:  elementType.defaultDepth  ?? 60,
      thicknessMM: elementType.thicknessMM??2,
      backThicknessMM: elementType.thicknessMM ??0.8,
      zocalo: elementType.zocalo,
      quantity: 1,
      layoutGroupId: selectedGroupId,
    });
  };

  const disabled = !selectedGroupId || adding;

  return (
    <div className={`group flex items-center justify-between rounded-md px-2 py-2 transition-colors ${
      selectedGroupId ? "hover:bg-gray-50 dark:hover:bg-gray-800/60" : "opacity-60"
    }`}>
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm text-gray-800 dark:text-gray-200">{elementType.name}</p>
        <p className="text-xs text-gray-400 dark:text-gray-500">
          {Number(elementType.basePrice).toLocaleString("es-CO", { style: "currency", currency: "COP", minimumFractionDigits: 0 })}
          {UNIT_SUFFIX[elementType.unit]}
        </p>
      </div>
      <button
        onClick={handleAdd}
        disabled={disabled}
        title={!selectedGroupId ? "Selecciona un grupo primero" : `Agregar ${elementType.name}`}
        className="ml-2 flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-gray-200
          text-gray-400 transition-all
          hover:border-gray-400 hover:text-gray-700
          disabled:cursor-not-allowed disabled:opacity-30
          dark:border-gray-700 dark:hover:border-gray-500 dark:hover:text-gray-300
          group-hover:opacity-100 opacity-0"
        style={{ opacity: selectedGroupId ? undefined : 0.3 }}
      >
        {adding
          ? <span className="h-3 w-3 animate-spin rounded-full border-2 border-gray-300 border-t-gray-600" />
          : <PlusIcon className="h-3.5 w-3.5" />
        }
      </button>
    </div>
  );
}
